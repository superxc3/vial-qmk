...
#else // defined(DIGITIZER_ENABLE) && defined(SPLIT_DIGITIZER_ENABLE)
...
static void dip_switch_handlers_slave(matrix_row_t master_matrix[], matrix_row_t slave_matrix[]) {
    split_shmem->dip_switch.state    = dip_switch_read_state();
    split_shmem->dip_switch.checksum = crc8(&split_shmem->dip_switch.state, sizeof(split_shmem->dip_switch.state));
}

// clang-format off
#    define TRANSACTIONS_DIP_SWITCH_MASTER() TRANSACTION_HANDLER_MASTER(dip_switch)
#    define TRANSACTIONS_DIP_SWITCH_SLAVE() TRANSACTION_HANDLER_SLAVE_AUTOLOCK(dip_switch)
...
#else // defined(DIP_SWITCH_ENABLE) && defined(SPLIT_DIP_SWITCH_ENABLE)

#    define TRANSACTIONS_DIP_SWITCH_MASTER()
#    define TRANSACTIONS_DIP_SWITCH_SLAVE()
#    define TRANSACTIONS_DIP_SWITCH_REGISTRATIONS

#endif // defined(DIP_SWITCH_ENABLE) && defined(SPLIT_DIP_SWITCH_ENABLE)

...
/* ============================================================================
 * sofleplus2ano — Legendary board, 58 LEDs (29 per-key per side, no underglow)
 *
 ...
 * board's own tps43-504:
 *   - I2C1 at 1 MHz and the 10 ms task throttle (unchanged from base)
 *   - the spinning disc on FIVE DIP pins (GP12-GP16, sofleplus2ano/config.h),
 ...
 *   - the swapped encoder entries in vial.json
 * The 58-LED EEPROM map is unchanged from base (the colour block ends at
 * 0x106F, directly below the indicator magic at 0x1070 -- no overlap).
 * ============================================================================ */
...
//#define I2C1_CLOCK_SPEED 100000   // 100kHz standard mode (what this board's TPS65 keymaps use)
#define I2C1_CLOCK_SPEED 1000000  // 1MHz FM+ — matches this board's tps43-504 and base tps43-510; halves I2C hold time per read, reducing OLED lag from shared bus contention

//#define AZOTEQ_IQS5XX_TPS65
#define AZOTEQ_IQS5XX_REPORT_RATE 9
//#define AZOTEQ_IQS5XX_ROTATION_270 /*for tps65*/
// Pure 10ms polling throttle (as this board's tps43-504). Honoured because DIGITIZER_MOTION_PIN
...
/* RDY/RST — MUST NOT be defined on sofleplus2ano.
 ...
 * (DIP_SWITCH_PINS in sofleplus2ano/config.h): quantum/digitizer.c would
 ...
 * instead, which is what this board's tps43-504 shipped on. */
...
#undef PRODUCT
#define PRODUCT "SoflePLUS2 TPS43 v5.10 Legendary"
 ...
 * sofleplus2ano uses the same allocation (per keymap variant, not per
 * board), so a sofleplus2ano and a sofleplus2 both flashed tps43-510 collide
 * on one host exactly as two sofleplus2 would. */
#undef  PRODUCT_ID
 ...
 * (the encoder entries are swapped, as on the ano's tps43-504), so the two
 * layouts must not share a UID. */
#ifdef VIAL_ENABLE
#define VIAL_KEYBOARD_UID {0x7B, 0xD0, 0x45, 0x9E, 0x23, 0x6C, 0xA1, 0xF8}
#endif
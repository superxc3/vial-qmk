...
// 0x0FC2-0x106F: g_direct_mode_colors (RGB_MATRIX_LED_COUNT * sizeof(HSV) = 58*3 = 174 bytes)
// 0x1070-0x1071: Indicator config magic (0x494Du = 'IM')
// 0x1072-0x10DF: indicator_config_t (110 bytes: role_masks[10]*8 + colors[10][3])
// 0x10E0-0x10E1: OLED config magic (0x4F4Cu = 'OL')
// 0x10E2-0x1123: oled_config_t (66 bytes)
//
// sofleplus2ano has 58 LEDs (29 per side, no underglow), the same count as
// sofleplus2, so this map is the base map unchanged: the colour block ends at
// 0x106F, directly below the indicator magic, and nothing overlaps. (The base
// keymap's comment describing a 72-LED overlap here is wrong for 58-LED boards;
// the overlap is real only on the 72-LED sofleplus2u / sofleplus2legendaryrgb,
// whose keymaps move the indicator and OLED blocks up to 0x10A0 / 0x1110.)
// The _Static_asserts below hold the arithmetic.
//
...
// dynamic-macro area (DYNAMIC_KEYMAP_EEPROM_MAX_ADDR 32767 in sofleplus2ano/config.h),
...
/* sofleplus2ano: the spinning disc is five switches on DIP pins GP12-GP16
 * (DIP_SWITCH_PINS in sofleplus2ano/config.h), not on matrix column 6.

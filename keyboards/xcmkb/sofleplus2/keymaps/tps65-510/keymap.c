...
//
// FIXED 2026-09-02 -- the sleep timeout lived at 0x10DC and would not survive a
// replug. 0x10DC was documented as a "reserved gap" on the basis that
// indicator_config_t ended at 0x10DB, but 0x1072 + 110 = 0x10E0, so the struct
// actually ends at 0x10DF and the timeout occupied its last four bytes. Every
// indicator save wrote colors[8][2]/colors[9][*] over it; the value read back at
// boot then failed the MIN/MAX range check and fell back to the 1-minute
// default. Moved to 0x0FB8, in the genuinely free 0x0FB6-0x0FBF gap.
//
// KNOWN, NOT YET FIXED -- the map above still overlaps in one place: the
// VialRGB direct-mode colors really run to 0x1099 (216 bytes for 72 LEDs; the
// old comment said 174, which is 58 LEDs), so they cover the indicator magic at
// 0x1070 and the first 42 bytes of indicator data. Saving per-key colors and
// saving indicator roles therefore corrupt each other. Fixing it means moving
// the indicator and OLED blocks above 0x1099, which resets everyone's saved
// indicator colours and OLED layer names once -- deliberately deferred.

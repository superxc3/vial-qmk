...
void keyboard_post_init_user(void) {
    // GP14 = IQS5xx RST. External 10k pull-up holds chip out of reset.
    // Do NOT pulse here: keyboard_post_init_user runs AFTER azoteq_iqs5xx_init,
    // so a reset would wipe the driver-applied config (rotation, gestures, etc.).

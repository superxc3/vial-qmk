...
void keyboard_post_init_user(void) {
    // v7.00nr RIGHT: GP12 = IQS5xx RST (active LOW).
    // Drive HIGH first (deassert), pulse LOW to hard-reset the chip, then wait for boot.
    // Without this, floating RST can hold IQS5xx in an unknown state → I2C init hangs → slave not connecting.
    gpio_set_pin_output(GP12);
    gpio_write_pin_high(GP12);   // deassert RST
    wait_ms(5);
    gpio_write_pin_low(GP12);    // assert RST (≥1ms pulse required by IQS5xx datasheet)
    wait_ms(2);
    gpio_write_pin_high(GP12);   // deassert RST
    wait_ms(100);                // IQS5xx boot time after RST released

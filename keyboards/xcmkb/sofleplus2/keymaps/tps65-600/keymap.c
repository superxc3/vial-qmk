...
// ==================== Auto-Layer: Raw HID ====================
// Protocol (usage page 0xFF60, usage 0x61 — standard QMK Raw HID):
//   0x30–0x39  → switch to layer 0–9  (blocked while any key is physically held)
//   0x40       → query current layer  (responds with layer number in data[0])
// VIA/Vial handles its own command IDs (0x01–0x13, 0xFE) first,
// then routes unknown IDs here via raw_hid_receive_kb().

#ifdef RAW_ENABLE
#include "raw_hid.h"
#include "matrix.h"

static bool auto_layer_any_key_held(void) {
...
void raw_hid_receive_kb(uint8_t *data, uint8_t length) {
    ...
    if (command == 0x40) {
        // Query: respond with current highest active layer
    ...
    if (command >= 0x30 && command <= 0x39) {
        // Switch to layer 0–9, blocked while a key is physically held
        if (!auto_layer_any_key_held()) {
            layer_move(command - 0x30);
        }
        return;
    }

    // Unknown command: mark as unhandled (VIA convention)
    data[0] = 0xFF;
}
#endif

...
// Array to track keycodes for 5 dip switches to ensure clean release
static uint16_t dip_switch_keycodes[5] = {KC_NO, KC_NO, KC_NO, KC_NO, KC_NO};

bool dip_switch_update_user(uint8_t index, bool active) {
    if (index >= 5) return true;

    // Mapping indices to your keyboard.json matrix positions
	keypos_t dip_positions[5] = {
			{ .row = 4, .col = 6 }, // Virtual Idx 0 (Physical Middle) -> Matrix Click [4,6]
			{ .row = 3, .col = 6 }, // Virtual Idx 1 (Physical Down)   -> Matrix Down  [3,6]
			{ .row = 2, .col = 6 }, // Virtual Idx 2 (Physical Right)  -> Matrix Right [2,6]
			{ .row = 1, .col = 6 }, // Virtual Idx 3 (Physical Up)     -> Matrix Up    [1,6]
			{ .row = 0, .col = 6 }  // Virtual Idx 4 (Physical Left)   -> Matrix Left  [0,6]
		};


    keypos_t kp = dip_positions[index];
    ...
    if (active) {
        ...
        keyrecord_t record = {
            .event = {.key = kp, .pressed = true, .time = timer_read(), .type = KEY_EVENT},
            .keycode = kc
        };
        ...
        if (kc == RM_TOGG) {
            ...
            rgb_matrix_step();
        } else if (kc >= QK_KB_0 && kc <= QK_KB_31) { // User keycodes/Macros
        ...
        if (kc != KC_NO) {
            keyrecord_t record = {
                .event = {.key = kp, .pressed = false, .time = timer_read(), .type = KEY_EVENT},
                .keycode = kc
            };

            if (IS_QK_MOMENTARY(kc)) {
                layer_off(QK_MOMENTARY_GET_LAYER(kc));
            } else if (kc >= QK_KB_0 && kc <= QK_KB_31) {
                process_record_user(kc, &record);

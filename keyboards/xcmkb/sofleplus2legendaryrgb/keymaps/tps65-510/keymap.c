 ...
 * sofleplus2legendaryrgb: 36 per side = 7 underglow (indices 0-6 / 36-42) + 29 per-key.
 * The slave sync packet is then 1 + 36*3 = 109 bytes, within RPC_M2S_BUFFER_SIZE 112. */
#define VIALRGB_SPLIT_LEFT 36u
...
// ==================== Custom Keycodes ====================
    ...
    CK_ATMWD,
    CK_PO,
    /* Kept in the Vial "Custom" keycode list — keep these three first so their
       QK_KB indices stay aligned with vial.json customKeycodes (positional map). */
    OS_DETECTION_TOGGLE,
    AP_GLOB,
    TP_INFO,
    /* Below: still handled in process_record_user, but hidden from the Vial GUI
       (these settings live in the Trackpad tab). Not listed in vial.json. */
    SCROLL_DIR_V,
    SCROLL_DIR_H,
    CURSOR_SPEED_UP,
    CURSOR_SPEED_DN,
...
/* v5.06: shared OLED + RGB sleep timeout (ms), runtime-settable from Vial and
 * persisted to EEPROM. Drives OLED sleep here and RGB sleep via the core
 * g_rgb_matrix_timeout (kept in sync by apply_sleep_timeout()). */
...
// 0x0FB8-0x0FBB: shared OLED+RGB sleep timeout, uint32 ms  (moved 2026-09-02, see below)
// 0x0FC0-0x0FC1: VialRGB colors magic (0x5243 = 'RC')
// 0x0FC2-0x1099: g_direct_mode_colors (RGB_MATRIX_LED_COUNT * sizeof(HSV) = 72*3 = 216 bytes)
// 0x109A-0x109F: free
// 0x10A0-0x10A1: Indicator config magic (0x494Du = 'IM')
// 0x10A2-0x110F: indicator_config_t (110 bytes: role_masks[10]*8 + colors[10][3])
// 0x1110-0x1111: OLED config magic (0x4F4Cu = 'OL')
// 0x1112-0x1153: oled_config_t (66 bytes)
//
// sofleplus2legendaryrgb (72 LEDs) -- the indicator and OLED blocks sit 0x30 higher
// than on sofleplus2. The base keymap keeps them at 0x1070/0x10E0, which is fine
// there (58 LEDs -> the colour block ends at 0x106F) but on this board the
// 216-byte colour block runs to 0x1099 and would cover the indicator magic and
// the first 42 bytes of indicator data, so saving per-key colours and saving
// indicator roles corrupted each other (this is what this board's tps65-504 did).
// Moving them costs a one-time reset of saved indicator colours and OLED layer
// names on first boot of this build; the magic words simply fail to match at
// the old addresses and the defaults are used.
//
// FIXED 2026-09-02 (inherited from base) -- the sleep timeout lived at 0x10DC,
// inside indicator_config_t, and was overwritten on every indicator save; it is
// at 0x0FB8, in the genuinely free 0x0FB6-0x0FBF gap.
//
// STILL DEFERRED (same as base): this whole region lies inside Vial's
// dynamic-macro area (DYNAMIC_KEYMAP_EEPROM_MAX_ADDR 32767 in sofleplus2legendaryrgb/config.h),
// so a very large macro set or a macro reset can wipe it. Same class of fix as
// sofleplus1 v1.10 (cap MAX_ADDR, move the block above it) -- a separate change.
...
#define EEPROM_INDICATOR_MAGIC_OFFSET      0x10A0u  /* 72-LED boards: above the colour block (base: 0x1070) */
#define EEPROM_INDICATOR_DATA_OFFSET       0x10A2u
#define INDICATOR_EEPROM_MAGIC             0x494Du  /* 'I','M' — Indicator bitmask v5 */
#define EEPROM_OLED_MAGIC_OFFSET           0x1110u  /* 72-LED boards: follows indicator block (base: 0x10E0) */
#define EEPROM_OLED_DATA_OFFSET            0x1112u
#define OLED_EEPROM_MAGIC                  0x4F4Cu  /* 'O','L' — OLED labels */

/* Compile-time guard for the map above: the colour block must end before the
 * indicator magic, and the indicator block before the OLED magic. */
_Static_assert(EEPROM_VIALRGB_COLORS_DATA_OFFSET + RGB_MATRIX_LED_COUNT * sizeof(HSV) <= EEPROM_INDICATOR_MAGIC_OFFSET,
               "VialRGB direct colours overlap the indicator block");
_Static_assert(EEPROM_INDICATOR_DATA_OFFSET + 110 <= EEPROM_OLED_MAGIC_OFFSET,
               "indicator block overlaps the OLED block");
#define EEPROM_SLEEP_TIMEOUT_OFFSET        0x0FB8u  /* uint32 shared sleep timeout (ms); was 0x10DC, which sat inside indicator_config_t */
...
static void indicator_config_init_defaults(void) {
    ...
    /* Default: one key per role — the ten thumb keys [4,4] [4,3] [4,2] [4,1] [4,0]
     * / [9,4] [9,3] [9,2] [9,1] [9,0], same physical keys as the base keymap's
     * {4,5,6,15,16,33,34,35,44,45}. sofleplus2legendaryrgb's LED chain starts with 7
     * underglow LEDs per half (indices 0-6 and 36-42), so the per-key indices
     * are +7 on the left and +14 on the right. */
    static const uint8_t default_leds[INDICATOR_NUM_ROLES] = {11, 12, 13, 22, 23, 47, 48, 49, 58, 59};
...
typedef struct {
    ...
    uint32_t sleep_timeout;   // v5.06: shared OLED+RGB sleep timeout (ms)
...
/* Sub-ID 0x53 — shared OLED+RGB sleep timeout (whole minutes on the wire). */
void vialrgb_get_power_settings_user(uint8_t *args) {
    args[0] = (uint8_t)(g_sleep_timeout / 60000u);
}

void vialrgb_set_power_settings_user(const uint8_t *args) {
    uint32_t ms = (uint32_t)args[0] * 60000u;
    if (ms < MIN_SLEEP_TIMEOUT_MS) ms = MIN_SLEEP_TIMEOUT_MS;
    if (ms > MAX_SLEEP_TIMEOUT_MS) ms = MAX_SLEEP_TIMEOUT_MS;
    g_sleep_timeout = ms;
    apply_sleep_timeout();
...
static void tp_info_sync_handler(uint8_t in_buflen, const void *in_data,
    ...
    if (in_buflen >= sizeof(tp_info_payload_t)) {
        ...
        /* v5.06: adopt master's shared sleep timeout so both halves sleep together */
...
#ifndef INDICATOR_BRIGHTNESS_PCT
#define INDICATOR_BRIGHTNESS_PCT 100   // 100 = no change; keymap config.h sets the real value
#endif

bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    // v5.06 update to standardise layer indicator colour brightness with the RGB
    // effects: scale to INDICATOR_BRIGHTNESS_PCT of the slider so a steady
    // indicator doesn't out-shine the (usually sub-ceiling) RGB effects.
    uint8_t brightness = rgb_matrix_get_val() * INDICATOR_BRIGHTNESS_PCT / 100;
     ...
     * sofleplus2legendaryrgb: role_masks are uint64 (the 0x45 Vial protocol carries
     * 8 bytes per role), so LEDs 64-71 -- the right half's two outer columns,
     * matrix [5..8,1] and [5..8,0] -- cannot be indicator LEDs on this 72-LED board. */
...
#else
    /* Fallback: hardcoded single-LED-per-role (72-LED indices: thumb keys, +7/+14 for underglow) */
    static const uint8_t fallback_leds[10]  = {11,  12,  13,  22,  23,  47,  48,  49,  58,  59};
...
/* sofleplus2legendaryrgb: the spinning disc is five switches on DIP pins GP12-GP16
 * (DIP_SWITCH_PINS in sofleplus2legendaryrgb/config.h), not on matrix column 6.
 * Each is mapped to the matrix position that LAYOUT and vial.json already
 * reserve for it and fed through action_exec(), so Vial remaps, layers, mods,
 * tap dance and combos all behave as for a real key -- the same mechanism the
 * v5.10 base uses for its single GP12 switch, replacing this board's 504-era
 * hand-rolled register/unregister handler. Indices are per hand since the
 * split DIP fix (quantum/dip_switch.h): 0-4 = left half, 5-9 = right half.
 * The disc is on the left half only; the right half's indices are ignored,
 * as the 504 handler did. */
static const keypos_t dip_positions[5] = {
    { .row = 4, .col = 6 }, // idx 0 (GP12, physical Middle) -> Click [4,6]
    { .row = 3, .col = 6 }, // idx 1 (GP13, physical Down)   -> Down  [3,6]
    { .row = 2, .col = 6 }, // idx 2 (GP14, physical Right)  -> Right [2,6]
    { .row = 1, .col = 6 }, // idx 3 (GP15, physical Up)     -> Up    [1,6]
    { .row = 0, .col = 6 }, // idx 4 (GP16, physical Left)   -> Left  [0,6]
};

bool dip_switch_update_user(uint8_t index, bool active) {
    if (index < ARRAY_SIZE(dip_positions)) {
        keyevent_t event = {
            .key = dip_positions[index],
            .pressed = active,
            .time = timer_read(),
            .type = KEY_EVENT
        };
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [2] = LAYOUT(
        ...
        KC_CAPS_LOCK,   LSFT(KC_5), LSFT(KC_6), KC_TRNS,    KC_TRNS,    LSFT(KC_8), KC_TRNS, KC_TRNS, KC_NUM_LOCK,  KC_1,   KC_2,   KC_3,   KC_KP_SLASH,    KC_KP_ENTER,
                ...
                KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, MS_BTN1   // v5.06: speed keycodes moved to Trackpad tab
    ),
...
void keyboard_post_init_user(void) {
    ...
    // ws2812_init() sets PAL_RP_PAD_SLEWFAST on GP0 (WS2812 DIN). Fast edges couple onto
    // adjacent GP2/GP3 I2C traces, corrupting IQS5xx reads → 10ms timeout stalls → RGB jitter.
    // This fires after ws2812_init and removes SLEWFAST (keeps PIO0 function, 2mA default).
    palSetLineMode(GP0, PAL_RP_GPIO_OE | PAL_MODE_ALTERNATE_PIO0);
    ...
    { /* v5.06: load persisted shared sleep timeout, clamp, then mirror to RGB */
        uint32_t st = eeprom_read_dword((uint32_t *)EEPROM_SLEEP_TIMEOUT_OFFSET);
        if (st >= MIN_SLEEP_TIMEOUT_MS && st <= MAX_SLEEP_TIMEOUT_MS) g_sleep_timeout = st;
    }
    apply_sleep_timeout();   // mirror shared sleep timeout to core RGB timeout
...
/* v5.11 FIX: apply the user's scroll-inversion setting.
 *
 * scroll_dir_v/h are saved to EEPROM (save_user_settings), exposed to the Vial
 * GUI as args[2]/[3] of the 0x50 trackpad-settings channel, and toggled by the
 * SCROLL_DIR_V / SCROLL_DIR_H keycodes -- but nothing ever read them back to
 * modify a report, so all three controls did nothing.
 *
 * Working code existed on the v1 pointing-device path
 * (sofleplus1/keymaps/vialazo102b/keymap.c:194,219) and was lost when that path
 * was replaced by the digitizer. Every sofleplus2 keymap from the 501b Archive
 * onward carries the dead pattern, so this was never a v5.10 regression -- the
 * toggle has been inert on every unit shipped.
 *
 * Called at BOTH exits of digitizer_pre_send_user so it covers both scroll
 * sources: the core's 2-finger scroll (already in report->v/h by the time we
 * are called -- digitizer_mouse_fallback.c:480-481 -- which is the early-return
 * path below) and the 1-finger scroll this function writes on a scroll layer.
 * Applying it only at the top would let the gesture block overwrite v/h and
 * silently drop the setting on exactly the layers where scrolling is the point.
 *
 * Independent of the compile-time DIGITIZER_SCROLL_INVERT in config.h, which
 * stays as the macOS natural-scrolling default. */
static inline void apply_scroll_invert(report_mouse_t *report) {
...
void digitizer_pre_send_user(report_mouse_t *report) {
    ...
    if (digitizer_active_contacts != 1) {
        ...
        apply_scroll_invert(report);   /* v5.11: covers the core 2-finger scroll */
        return;
    }
    ...
    apply_scroll_invert(report);   /* v5.11: covers the 1-finger scroll written above */
}
...
__attribute__((unused)) static const char* get_trackpad_gesture_bitmap(uint8_t layer) {
...
__attribute__((unused)) static const char* get_trackpad_gesture_bitmap_by_mode(uint8_t mode) {
    switch (mode) {
...
static void print_status_narrow(void) {
    ...
    if (is_slave && !g_slave_sync_valid) {
        ...
        oled_write_P(PSTR("     "), false);   // v5.06: blank when no OS to show (OS detection off / PTP / not yet detected); SYNC is only the slave-awaiting-sync state
    } else {

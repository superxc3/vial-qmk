 ...
 * sofleplus2u: 36 per side = 7 underglow (indices 0-6 / 36-42) + 29 per-key.
...
//
// sofleplus2u (72 LEDs) -- the indicator and OLED blocks sit 0x30 higher than
// on sofleplus2. The base keymap keeps them at 0x1070/0x10E0, which is fine
...
// indicator roles corrupted each other (this is what the u's tps65-504 did).
...
// dynamic-macro area (DYNAMIC_KEYMAP_EEPROM_MAX_ADDR 32767 in sofleplus2u/config.h),
...
#define EEPROM_INDICATOR_MAGIC_OFFSET      0x10A0u  /* u: above the 72-LED colour block (base: 0x1070) */
#define EEPROM_INDICATOR_DATA_OFFSET       0x10A2u
#define INDICATOR_EEPROM_MAGIC             0x494Du  /* 'I','M' — Indicator bitmask v5 */
#define EEPROM_OLED_MAGIC_OFFSET           0x1110u  /* u: follows indicator block (base: 0x10E0) */
...
static void indicator_config_init_defaults(void) {
     ...
     * {4,5,6,15,16,33,34,35,44,45}. sofleplus2u's LED chain starts with 7
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
     ...
     * sofleplus2u: role_masks are uint64 (the 0x45 Vial protocol carries 8 bytes
     * per role), so LEDs 64-71 -- the right half's two outer columns, matrix
     * [5..8,1] and [5..8,0] -- cannot be indicator LEDs on this 72-LED board. */
...
#else
    /* Fallback: hardcoded single-LED-per-role (u indices: thumb keys, +7/+14 for underglow) */

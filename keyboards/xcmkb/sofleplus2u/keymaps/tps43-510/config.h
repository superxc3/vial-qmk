...
/* ============================================================================
 * sofleplus2u — Signature RGB board (72 LEDs: 7 underglow + 29 per-key per side)
 *
 * Port of sofleplus2/keymaps/tps43-510 (v5.10 feature set: IDLE_RESEED +
 * CONFIDENCE_GATE phantom fix, OS_DETECTION_KEYBOARD_RESET removed, shared
 * OLED+RGB sleep timeout, OLED labels, scroll-inversion fix, PID 0x0289).
 * Kept from the u's own tps43-504:
 *   - I2C1 at 1 MHz and the 10 ms task throttle (unchanged from base)
 *   - no RDY/RST pin defines (GP13/GP14 are not wired on this batch; the u's
 *     tps43-504 already ran pure timed polling)
 *   - VIALRGB_SPLIT_LEFT 36 and the swapped encoder entries in vial.json
...
//#define I2C1_CLOCK_SPEED 100000   // 100kHz standard mode (what the u's TPS65 keymaps use)
#define I2C1_CLOCK_SPEED 1000000  // 1MHz FM+ — matches u tps43-504 and base tps43-510; halves I2C hold time per read, reducing OLED lag from shared bus contention

//#define AZOTEQ_IQS5XX_TPS65
#define AZOTEQ_IQS5XX_REPORT_RATE 9
//#define AZOTEQ_IQS5XX_ROTATION_270 /*for tps65*/
// Pure 10ms polling throttle (as u tps43-504). Honoured because DIGITIZER_MOTION_PIN
// is NOT defined below — see the RDY/RST note.
...
/* RDY/RST — deliberately NOT defined on sofleplus2u.
 * The base tps43-510 defines DIGITIZER_MOTION_PIN GP13 / TRACKPAD_RST_PIN GP14
 * for hand-wired IQS5xx RDY/RST lines and builds trackpad_rdy_rst.c. This
 * batch has neither pin connected: defining DIGITIZER_MOTION_PIN would make
 * quantum/digitizer.c #undef DIGITIZER_TASK_THROTTLE_MS and pace reads from a
 * floating GP13, and would run up to three bogus boot-time hard-reset + re-ATI
 * cycles on a chip that never sees the RST line. The u's tps43-504 shipped on
 * exactly this timed-polling path. If a unit is ever hand-wired, add the two
 * defines and `SRC += trackpad_rdy_rst.c` back from the base keymap. */
...
#undef PRODUCT
#define PRODUCT "SoflePLUS2 TPS43 v5.10 Signature RGB"

/* USB Product ID -- distinct per variant so several boards can be plugged in at
 * once. builddefs/build_vial.mk force-defines SERIAL_NUMBER="vial:f64c2b3c" on
 * EVERY Vial build, and usb_descriptor.c prefers that static string over the
 * per-MCU hardware ID, so every board reports the same serial. Windows collapses
 * identical VID+PID+serial into one device instance, which is why a second board
 * silently fails to appear. The serial cannot be changed -- the Vial app finds a
 * keyboard by the "vial:" prefix -- so the PID is the only lever that does not
 * require editing core.
 *
 * Allocation: 0x0287 board default = tps65-510 (vertical), 0x0288 = tps65-510h
 * (horizontal), 0x0289 = tps43-510. SCOPE: the PID is in the FIRMWARE, not the
 * unit, so two boards both flashed tps65-510h still collide; per-unit
 * coexistence needs a unique serial, i.e. a separate build per board.
 * Keymap config.h is last in QMK's config chain, so it overrides keyboard.json.
 * sofleplus2u uses the same allocation (per keymap variant, not per board), so
 * a sofleplus2u and a sofleplus2 both flashed tps43-510 collide on one host
 * exactly as two sofleplus2 would. */
#undef  PRODUCT_ID
#define PRODUCT_ID 0x0289



/* Vial UID: this board's own. Its vial.json differs from sofleplus2/tps43-510
 * (the encoder entries are swapped, as on the u's tps43-504), so the two
 * layouts must not share a UID. */
#ifdef VIAL_ENABLE
#define VIAL_KEYBOARD_UID {0xD6, 0x7C, 0xC0, 0x7B, 0x02, 0x51, 0x70, 0x08}
#endif